...
public class Steak {
	...
	/******** Static Variables **************/
	private static int counter; //Track number of steak objects displayed
	...
	public Steak() {
		...
		this(Doneness.RAW, 8, SteakType.RIBEYE, 0);
	}
	
	//Copy Constructor
	public Steak(Steak s) {
		this(s.doneness, s.ounces, s.type, s.price);
	}
	...
	public Steak(Doneness doneness, int ounces, SteakType type, double price  ) {
		...
		counter++; //All other constructors call this one, so only increment here
	}
...
/**************** Methods *****************/
	...
	//Display count of Steak objects created to stdout
	public static void showCount() {
		System.out.printf("The total number of steaks made so far is: %d%n", counter);
	}
	...
	public String toString() {
		...
		ret = this.doneness + " " + this.ounces + " " + this.type + " for $" + df.format(this.price);
		return ret;
	}
	...
	public boolean equals(Object obj) {
		...
		if (obj instanceof Steak) { 
			...
			if(this.doneness == steak.getDoneness() && this.ounces == steak.getOunces() && this.type == steak.getType() && this.price == steak.getPrice()){
				ret = true;
			}
		}
		return ret;
